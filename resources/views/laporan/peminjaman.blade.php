...
<body class="bg-gray-100 text-gray-800 min-h-screen font-sans">
            ...
            <div class="max-w-6xl mx-auto bg-white rounded-xl shadow-md overflow-hidden p-6">
                ...
                <div class="flex justify-between items-center mb-8 border-b border-gray-200 pb-4">
                    <div>
                        ...
                        <p class="text-gray-600 mt-1">Daftar lengkap peminjaman barang</p>
                    </div>
                    ...
                    <table class="min-w-full text-sm bg-white">
                        <thead>
                            <tr class="bg-gradient-to-r from-primary-100 to-primary-50 text-primary-800">
                                ...
                                <th class="px-6 py-4 text-left font-semibold tracking-wider">Tanggal Pinjam</th>
                        ...
                        <tbody class="divide-y divide-gray-200">
                            ...
                            <tr class="hover:bg-primary-50 transition-all">
                                ...
                                <td class="px-6 py-4 text-gray-700">{{ $item->tanggal_pinjam }}</td>
                                <td class="px-6 py-4">
                                    @if($item->status == 'dikembalikan')
                                        <span class="bg-green-100 text-green-800 text-xs py-1 px-3 rounded-full font-medium">Dikembalikan</span>
                                    @elseif($item->status == 'dipinjam')
                                        <span class="bg-yellow-100 text-yellow-800 text-xs py-1 px-3 rounded-full font-medium">Dipinjam</span>
                                    @elseif($item->status == 'terlambat')
                                        <span class="bg-red-100 text-red-800 text-xs py-1 px-3 rounded-full font-medium">Terlambat</span>
                                    @else
                ...
                <div class="mt-8 pt-6 border-t border-gray-200 text-sm text-gray-500 flex justify-between items-center">
                    <div class="flex items-center gap-4">
                        <div class="flex items-center">
                            ...
                            Total Peminjaman: <span class="font-semibold ml-1">{{ count($peminjaman) }}</span>
                        </div>
                        <div class="flex items-center">
                            <i class="fas fa-chart-bar mr-2 text-primary-500"></i>
                            Aktif: <span class="font-semibold ml-1">{{ $peminjaman->where('status', 'dipinjam')->count() }}</span>
                        </div>
                    </div>
                    <div class="flex items-center">
    ...
    <script>
        function downloadExcel() {
            ...
            const excelData = [
                ['No', 'Nama Peminjam', 'Barang', 'Tanggal Pinjam', 'Status']
            ];
            ...
            rows.forEach((row, index) => {
                ...
                excelData.push([no, namaPeminjam, barang, tanggalPinjam, status]);
            });
